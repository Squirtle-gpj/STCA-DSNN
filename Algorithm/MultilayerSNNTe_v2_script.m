% function Result = MultilayerSNNTe_v2(Data,Structure,testGroup)
    tic
...
%                       gpuArray(int32(Structure.Layers(ilayer))),gdecay1(ilayer - 1),gdecay2(ilayer - 1));
%                   else
                      int_param = [gT_size,Structure.Layers(ilayer)];
...
%                   end
           ...
           output_spike_timing = [output_spike_timing;Tspike,pp*ones(length(Tspike),1),ineuron];
...
%                            tmp2 = findpeaks(gather(u{nLayers}(:,ioutput)));
%                            tmp(icls) = tmp(icls) + max(tmp2);
%                        end
%                    end
%                
%            end
