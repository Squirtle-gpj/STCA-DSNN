function Structure = MultilayerSNNTr(Data,ValData,nGroups,maxEpoch,Layers,startEpoch,existweights,curStructure,path,tau_m,dropout)
    tic
    ...
    if existweights == 0
        ...
        for ilayer = 2:nLayers
            Structure.AllWeights{ilayer} = gpuArray(double(...
                1e-3*randn(Structure.gatherLayers(ilayer-1),gather(Structure.gatherLayers(ilayer)))));
        end
           if writeErrlog == 1
            Errlog = cell(4,1);
           end
...
%                        tmp = sum(dVdinput(:,find(direction == 1),:),2) - sum(dVdinput(:,find(direction == -1),:),2);
                       tmp = (output{ilayer - 1}' * dVdinput).*V0;
...
%                        disp(1)
                       ...
                       if ilayer == (nLayers-1)
                           dVdO_lastlayer.GridSize = [ceil(nDrop(ilayer)/nThreadsperBlock_sqrt),ceil(nDrop(nLayers)/nThreadsperBlock_sqrt),t_alter_max];
                           dVdO = feval(dVdO_lastlayer,dVdO,dVdinput,Structure.AllWeights{ilayer+1}(Drop{ilayer},Drop{ilayer+1}),t_alter,V0,[gT_size,nDrop(ilayer),nDrop(nLayers)]);
                       else
                           dVdO_hiddenlayer.GridSize = [ceil(nDrop(ilayer)/nThreadsperBlock_sqrt),ceil(nDrop(nLayers)/nThreadsperBlock_sqrt),t_alter_max];
                           dVdO = feval(dVdO_hiddenlayer,dVdO,dVdinput,Structure.AllWeights{ilayer+1}(Drop{ilayer},Drop{ilayer+1}),t_alter,V0,[gT_size,nDrop(ilayer),nDrop(ilayer+1),nDrop(nLayers)]);
                       end
                       dVdM_hiddenlayer.GridSize = [ceil(t_alter_max/2),ceil(nDrop(ilayer)/nThreadsperBlock),nDrop(nLayers)];
                       dVdM = gpuArray(zeros(T_size,nDrop(ilayer),nDrop(nLayers),'double'));
                       dVdS = gpuArray(zeros(T_size,nDrop(ilayer),nDrop(nLayers),'double'));
                       [dVdM,dVdS] = feval(dVdM_hiddenlayer,dVdM,dVdS,dVdO,h,t_alter,[gthreshold,gdecay1,gdecay2],[gT_size,nDrop(ilayer),nDrop(nLayers)]);
                       
                       clear h dVdO dVdinput
                       dVdinput = dVdM - dVdS;
                       clear dVdM dVdS
                       tmp = sum(dVdinput,3);
                       tmp = (output{ilayer - 1}' * squeeze(tmp)).*V0;
                       Structure.AllWeights{ilayer }(Drop{ilayer-1},Drop{ilayer}) = Structure.AllWeights{ilayer }(Drop{ilayer-1},Drop{ilayer}) + lmd*tmp;
                       clear tmp 
%                        disp(2)
                   end
...
%       lmd = lmd*0.9;
      Result = MultilayerSNNTe(ValData,Structure,nGroups);
