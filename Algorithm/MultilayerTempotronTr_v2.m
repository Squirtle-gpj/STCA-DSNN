function Structure = MultilayerTempotronTr_v2(Data,ValData,nGroups,maxEpoch,Layers,startEpoch,existweights,curStructure,path,tau_m,dropout)
    tic
...
%     maxsteps = getMaxsteps(tau_m,dt,1e-3);
% %     maxsteps = 500;
%     tau_s = tau_m/4;
%     
...
%     dVdM_hiddenlayer.ThreadBlockSize = [nThreadsperBlock,1,1];
    dVdinput_lastlayer = parallel.gpu.CUDAKernel('dVdinput_lastlayer_m1.ptx','dVdinput_lastlayer_m1.cu');
...
%                       [output{ilayer},u{ilayer},t_alter,direction] = feval(t_v_alter,output{ilayer},u{ilayer},t_alter, direction,cur_Groups,input{ilayer},int_param,param);
                      ...
                      [u{ilayer},t_alter,direction] = feval(t_v_alter,u{ilayer},t_alter, direction,input{ilayer},cur_Groups,gT_size,...
                           Structure.Layers(ilayer),gdecay1(ilayer),gdecay2(ilayer),gthreshold,subthreshold);
                  else
                      int_param = [gT_size,nDrop(ilayer)];
                      param = [gthreshold,gdecay1(ilayer),gdecay2(ilayer)];
...
%                        dVdM = gpuArray(zeros(T_size,nDrop(ilayer),'double'));
                       ...
                       dVdinput = feval(dVdinput_lastlayer,dVdinput,h,t_alter,direction,[gthreshold,gdecay1(ilayer),gdecay2(ilayer)],[gT_size,nDrop(ilayer),maxsteps(ilayer)]);
...
%                        disp(1)
                       ...
                       if ilayer == (nLayers-1)
                           dVdO_lastlayer.GridSize = [ceil(nDrop(ilayer)/nThreadsperBlock_sqrt),ceil(nDrop(nLayers)/nThreadsperBlock_sqrt),max(maxsteps(ilayer),t_alter_max)];
                           dVdO = feval(dVdO_lastlayer,dVdO,dVdinput,Structure.AllWeights{ilayer+1}(Drop{ilayer},Drop{ilayer+1}),t_alter,V0,[gT_size,nDrop(ilayer),nDrop(nLayers),maxsteps(ilayer)]);
                       else
                           dVdO_hiddenlayer.GridSize = [ceil(nDrop(ilayer)/nThreadsperBlock_sqrt),ceil(nDrop(nLayers)/nThreadsperBlock_sqrt),max(maxsteps(ilayer),t_alter_max)];
                           dVdO = feval(dVdO_hiddenlayer,dVdO,dVdinput,Structure.AllWeights{ilayer+1}(Drop{ilayer},Drop{ilayer+1}),t_alter,V0,[gT_size,nDrop(ilayer),nDrop(ilayer+1),nDrop(nLayers),maxsteps(ilayer)]);
                       end
...
%                        dVdS = gpuArray(zeros(T_size,nDrop(ilayer),nDrop(nLayers),'double'));
                       dVdinput = feval(dVdinput_hiddenlayer,dVdinput,dVdO,h,t_alter,[gthreshold,gdecay1(ilayer),gdecay2(ilayer)],[gT_size,nDrop(ilayer),nDrop(nLayers),maxsteps(ilayer)]);
...
%       lmd = lmd*0.9;
      Result = MultilayerTempotronTe_v2(ValData,Structure,nGroups);
